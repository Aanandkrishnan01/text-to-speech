...
subgraph pipeline["batch pipeline"]
  synthesize["manifest synthesizer"]
  model["ai model"]

end

out["output .wav files"]
combined["combined track and timestamps"]
flc["mp3 and flac files"]
...
batch-->db
db-->|read entries|synthesize
synthesize-->|load model once at start|model
synthesize-->|analyse tag priority loop process clips|synthesize

model-->out
aimodel-->|save wav|out
model-->|batch save|combined
model-->|batch save|flc
